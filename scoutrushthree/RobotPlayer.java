...
public strictfp class RobotPlayer {
		...
		try {
			while (true) {
				...
				if (reconMode) {
					...
					//If we have no current destination, move randomly
					if (destination == null) {
						...
						//Otherwise try a new random direction
						else {
					...
					//If we have a destination, try to move towards it
					else {
						
						//If we've reached the destination then we're done
						if (rc.canSenseLocation(destination)) {
							...
							//If we can't move directly to the destination, try to skirt around obstacles
							else {
					...
					if (enemies.length != 0) {
						if (rc.canFireSingleShot()) {
							rc.fireSingleShot(rc.getLocation().directionTo(getHighestPriorityTarget(enemies).location));
						}
					}
	...
	/*
	 * Given an array of enemy robots, this method will return the
	 * robot with the highest priority to be shot. Currently, this is
	 * calculated by:
	 * 
	 * Priority = Health / Attack
	 * 
	 * In the scenario where all attacks always hit their intended targets
	 * and all units have the same range (basically all of the units line up
	 * and start firing at each other) this strategy is optimal in that it
	 * leads to the least amount of damage taken by our team.
	 * 
	 * TODO: Account for range, bullet speed
	 */
	static RobotInfo getHighestPriorityTarget(RobotInfo[] enemies){
		if(enemies.length == 0){
			return null;
		}
