...
public class BroadcastManager {
    ...
    /*
    Represents a location. Locations stored:
    ENEMY: Location of a found enemy on the map
    ARCHON_HELP: Location of an archon if the archon sends a help request (i.e. this location is nonzero)
    GARDENER_HELP: Location of a gardener if the gardener sends a help request (i.e. this location is nonzero)
     */
    public enum LocationInfoType {
        ENEMY(0, 1, 20),
        ARCHON_HELP(21, 22, 24),
        GARDENER_HELP(25, 26, 75);

        // Represents the index that stores the pointer (i.e. index) of the most recently added/modified location
        int locationPointerIndex;
        // Represents the index start of the "block" that stores locations (inclusive)
        int locationStartIndex;
        // Represents the index end of the "block" that stores locations (inclusive)
    ...
    /*
    Represents the count of a specific unit.
    All unit types can be written directly except for ALLY_ROBOT_TOTAL and ALLY_ENEMY_TOTAL - these are
    ...
    public static void saveLocation(RobotController rc, MapLocation location, LocationInfoType type) throws GameActionException {
    ...
    public static MapLocation getRecentLocation(RobotController rc, LocationInfoType type) throws GameActionException {
        int val = rc.readBroadcast(type.locationPointerIndex);
        if (val == 0) {
            return null;
        }
    ...
    private static boolean isAllyRobot(UnitCountInfoType type) {
        if(type == UnitCountInfoType.ALLY_ROBOT_TOTAL || type == UnitCountInfoType.ALLY_ARCHON || type == UnitCountInfoType.ALLY_GARDENER ||
                type == UnitCountInfoType.ALLY_LUMBERJACK || type == UnitCountInfoType.ALLY_SCOUT || type == UnitCountInfoType.ALLY_SOLDIER
    ...
    private static boolean isEnemyRobot(UnitCountInfoType type) {
        if(type == UnitCountInfoType.ENEMY_ROBOT_TOTAL || type == UnitCountInfoType.ENEMY_ARCHON || type == UnitCountInfoType.ENEMY_GARDENER ||
                type == UnitCountInfoType.ENEMY_LUMBERJACK || type == UnitCountInfoType.ENEMY_SCOUT || type == UnitCountInfoType.ENEMY_SOLDIER
    ...
    /*
    Unzips an array of values from a given channel value.
    Uses the inverse of Cantor's recursive pairing function.
    @param numValuesToUnzip: the number of values that are expected to be unzipped.
     */
